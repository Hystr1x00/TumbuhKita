...
export default function AddChildModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await Swal.fire({
      ...
      text: 'Data anak akan ditambahkan ke sistem Posyandu',
      icon: 'question',
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        await fetchData.addAnakPosyandu(newChildData);
        ...
        Swal.fire({
          icon: 'error',
          title: 'Gagal!',
          text: 'Terjadi kesalahan saat menambahkan data anak',
          confirmButtonColor: '#6C2BD7'
        });
      } finally {
        setLoading(false);
    ...
    <div className="fixed inset-0 backdrop-blur-sm bg-white/30 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl p-6 w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto border border-gray-200">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-secondary-50">Tambah Anak Baru</h2>
          <button
            onClick={onClose}
