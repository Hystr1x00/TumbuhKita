...
export default function EditImmunizationModal({ isOpen, onClose, onSuccess, jadwal }) {
  const [formData, setFormData] = useState({
    judul: jadwal?.judul || '',
    status: jadwal?.status || 'Terjadwal',
    target: jadwal?.target || '',
    tanggal: jadwal?.tanggal || '',
    anakTerjadwal: jadwal?.anakTerjadwal || '',
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await Swal.fire({
      title: 'Update Jadwal Imunisasi?',
      text: 'Jadwal imunisasi akan diperbarui di sistem Posyandu',
      icon: 'question',
    ...
    if (result.isConfirmed) {
      ...
      try {
        // Update immunization schedule data
        ...
        Swal.fire({
          icon: 'success',
          title: 'Berhasil!',
          text: 'Jadwal imunisasi berhasil diperbarui',
          confirmButtonColor: '#6C2BD7'
        });
        ...
        Swal.fire({
          icon: 'error',
          title: 'Gagal!',
          text: 'Terjadi kesalahan saat memperbarui jadwal imunisasi',
          confirmButtonColor: '#6C2BD7'
        });
      } finally {
        setLoading(false);
    ...
    <div className="fixed inset-0 backdrop-blur-sm bg-white/30 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl p-6 w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto border border-gray-200">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-secondary-50">Edit Jadwal Imunisasi</h2>
          <button
            onClick={onClose}
